...
export default function MinibusParking() {
  const {
    floors,
    selectedFloor,
    setSelectedFloor,
  ...
  const minibusFloors = floors.filter(floor => floor.type === 'minibus');

  const initialMinibusFloor = minibusFloors.length > 0 ? minibusFloors[0].number : 1;
  const currentMinibusFloor = minibusFloors.find(floor => floor.number === selectedFloor) ? selectedFloor : initialMinibusFloor;

  const currentSlots = getSlotsByFloor(currentMinibusFloor);

  const selectedFloorData = floors.find(floor => floor.number === currentMinibusFloor);

  const totalSlots = selectedFloorData?.totalSlots || 0;
  const occupiedSlots = selectedFloorData?.occupiedSlots || 0;
  const revenue = 4500.00; // Mock revenue for minibus parking
  const securityIncidents = 0;
  const availableValets = 2;
  ...
  return (
    <div style={{ padding: '20px' }}>
      <div style={{ marginBottom: '20px' }}>
        <Title level={2} style={{ marginBottom: '8px' }}>Minibus Parking Management</Title>
        <p style={{ color: 'rgba(0, 0, 0, 0.45)' }}>
          100,000 sq ft facility with 30 minibus parking slots per floor and 3 gates
        </p>
      </div>
      ...
      <ParkingGrid
        slots={currentSlots}
        title={`Minibus Parking - Floor ${currentMinibusFloor}`}
        onSlotClick={handleSlotClick}
        selectedFloor={currentMinibusFloor}
        onFloorChange={setSelectedFloor}
        floors={minibusFloors}
        floorType="minibus"
      />
